...
// Сколько ударного времени после проноса следующий замах ещё считается
// продолжением серии. Больше суммы отката с возвратом (0.12 + 0.16),
// чтобы честный быстрый темп попадал в связку, но меньше секунды:
// выждал — начинаешь заново, с открывающего.
...
//
// Открывают удар только широкий и диагональ: оба начинаются у правого
// бока, где дубина и так висит. Обратный и рубящий сами по себе
// не выпадают — они живут в СВЯЗКЕ (см. выбор манеры в Guard).
...
export class SwingAction {
  constructor() {
    ...
    /** Часы удара в ударном времени — ими меряется окно связки. */
    this.age = 0;
    /** Когда закончился последний пронос (по тем же часам). */
    this.lastStrikeEnd = -1e9;
    /** Позиция в связке: 0 — открывающий, 1 — обратный, 2 — рубящий. */
    this.combo = 0;
    /** Прошлый открывающий: два одинаковых подряд читаются метрономом. */
    this.lastOpener = 3;
    ...
    switch (this.state) {
      case SwingState.Guard:
        ...
        if (this.held && this.cooldown <= 0) {
          ...
          this.timer = 0;
          // Быстрые клики нанизываются в СВЯЗКУ:
          //
          //   горизонтальный → обратный → рубящий сверху → заново.
          //
          // Обратный сам по себе не выпадает — из стойки правая рука
          // бэкхендом не бьёт, он появляется только как обратный ход
          // после удара влево, когда дубина уже на левой стороне.
          // Рубящий — финал серии: после обратного дубина справа,
          // взлёт над головой продолжает её ход. Пауза дольше окна
          // рвёт связку, и серия начинается с открывающего.
          if (this.age - this.lastStrikeEnd > COMBO_WINDOW) this.combo = 0;
          if (this.combo === 0) {
            // Открывающих два — широкий и диагональ, оба от правого бока.
            // Чередуются: один и тот же дважды подряд читается метрономом.
            this.styleIndex = this.lastOpener === 0 ? 3 : 0;
            this.lastOpener = this.styleIndex;
          } else {
            this.styleIndex = this.combo === 1 ? 1 : 2;
          }

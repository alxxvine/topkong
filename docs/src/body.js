...
export class Body {
  constructor(arena) {
    ...
    /**
     * Какие частицы ушли ПОД настил.
     *
     * Флаг, а не проверка по месту, и это принципиально. Под диск можно
     * попасть единственным способом — обогнув кромку, и с этого момента
     * частица падает мимо арены, сколько бы она ни оказалась потом
     * «внутри радиуса» по горизонтали. Пробовал обойтись порогом глубины:
     * подброс он вылечил, но взамен тело у края стало проваливаться
     * НАСКВОЗЬ — замерено, частицы уходили на 13 метров вниз, оставаясь
     * по горизонтали над настилом, а раз в несколько падений боец ещё
     * и вставал из-под пола.
     */
    this.underDeck = new Array(COUNT).fill(false);
  }
  ...
  step(dt) {
    ...
    this.invMass[P.ClubTip] = 1 / (this.clubOn ? MASS[P.ClubTip] : CLUB_OFF_MASS);
  ...
  groundResponse(dt) {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      // Тереться можно только о то, чего касаешься: ушедшая за кромку
      // частица летит мимо настила, а не по нему.
      if (this.underDeck[i]) continue;
  ...
  touchesDeck() {
    for (let i = 0; i < COUNT; i++) {
      ...
      // Ушедшая под диск частица опорой не является: под ней пустота.
      if (this.underDeck[i]) continue;
  ...
  /**
   * Пересчитать, кто сейчас под настилом.
   *
   * Уйти под диск можно только за кромкой: там под уровнем настила пустота.
   * Вернуться — только поднявшись выше его поверхности. Между этими двумя
   * событиями частица настила не касается вовсе, что бы ни говорила
   * её горизонтальная координата.
   */
  updateUnderDeck() {
    for (let i = 0; i < COUNT; i++) {
      const p = this.pos[i];
      if (this.underDeck[i]) {
        if (p.y >= RADIUS[i]) this.underDeck[i] = false;
      } else if (p.y < RADIUS[i] && !this.arena.isOverDeck(p.x, p.z)) {
        this.underDeck[i] = true;
      }
    }
  }

  /**
   * Ушёл ли под диск САМ ТАЗ.
   *
   * Условие намеренно самое узкое из трёх, которые я перепробовал.
   * По любой частице — тело, зацепившееся за кромку одной ногой, не вставало
   * и не падало ВООБЩЕ: два падения из шести висели весь замер. По корпусу
   * целиком — пять из шести. Причина одна: тело, легшее поперёк кромки,
   * держится тем, что осталось на настиле, и само по себе это устойчиво,
   * разрешаться там нечему. Значит запрет должен ловить ровно тот случай,
   * ради которого он и заводился, — таз в пустоте, из которой подъём
   * выдёргивает всё тело разом.
   */
  coreUnderDeck() {
    return this.underDeck[P.Hips];
  }
   ...
   */
  clampGround() {
    this.updateUnderDeck();
    for (let i = 0; i < COUNT; i++) {
      const p = this.pos[i];
      if (p.y >= RADIUS[i]) continue;
      // Ушедшее за кромку не выталкиваем НИ НА СКОЛЬКО: оно уже под диском.
      if (this.underDeck[i]) continue;
      if (!this.arena.isOverDeck(p.x, p.z)) continue;
      const q = this.prev[i];
  ...
  /**
   * Погасить ОБЩИЙ горизонтальный снос тела.
   *
   * Нужен встающему. Слабые мышцы вместе с трением о настил работают
   * храповиком: мышца тянет конечности к позе, возврат этой тяги двигает
   * всё тело, но прижатые к полу точки на месте держит трение — и тело
   * ползёт. Замерено: поднимающийся после тарана уезжал на три метра,
   * причём ПРОТИВ направления толчка, а заодно не мог успокоиться —
   * ползущее тело не проходит проверку на покой, и сила не растёт.
   *
   * Гасится только СРЕДНИЙ снос: относительное движение частиц не тронуто,
   * поза складывается как складывалась — тело просто держится за пол.
   */
  dampPlanar(rate, dt) {
    ...
    let vz = 0;
    for (let i = 0; i < COUNT; i++) {
      vx += this.pos[i].x - this.prev[i].x;
      vz += this.pos[i].z - this.prev[i].z;
    }
    vx /= COUNT;
    vz /= COUNT;
    for (let i = 0; i < COUNT; i++) {

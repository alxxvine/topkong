...
//
// Радиус стопы ОБЯЗАН совпадать с высотой, на которую её ставит поза,
// поэтому он не число, а сама эта высота. Пока они расходились — поза
// просила 0.054, а настил держал на 0.10, — прибитая к настилу стопа
// каждый кадр оказывалась на 4.6 см выше цели. Мышца тянет тело вниз,
// настил толкает стопу вверх, и решателю остаётся разводить их вбок
// по сфере вокруг стопы: ровно отсюда боец и уезжал сам.
const RADIUS = [
  0.20, 0.24, 0.19,
  0.12, 0.11, Rig.FootY,
  0.12, 0.11, Rig.FootY,
...
// Конечности держат позу КРЕПКО, голова и плечи мягко.
//
// Мягкие звенья хороши были для цельного мягкого тела, но у картонной куклы
// панель нарисована по кости, а кость стоит там, где оказалась частица.
// Не дотянула частица до цели — панели разошлись и вошли друг в друга.
// Замерено на прежних значениях: кисть отставала от цели на 15.6 см,
// локоть на 12 — тело именно что мялось.
//
// Голову и плечи оставляем слабыми: они не образуют длинных цепей и портить
// им нечего, зато на них держится вся живость корпуса.
const MUSCLE = [
  0.40, 1.00, 1.30,
  1.20, 1.10, 1.10,
  1.20, 1.10, 1.10,
...
// Сила мышцы для ТРЕТЬЕЙ итерации тела: жёсткие панели на верёвках.
//
// Замысел другой, чем у мышечного тела. Панель — картонка, она не гнётся
// и не мнётся; гнутся только суставы, и держит их верёвка. Поэтому корпус
// и то, чем боец правит — таз, грудь, плечи, бёдра, стопы — держится крепко,
// а дальние концы конечностей почти отпущены: локоть и кисть висят
// на своей верёвке и болтаются от ходьбы, разворота и удара.
//
// Колену немного силы всё же нужно: совсем свободное, оно выгибается назад,
// потому что цепь бедро→стопа одинаково хорошо складывается в обе стороны.
const MUSCLE_ROPE = [
  0.90, 1.30, 1.50,
  1.40, 0.30, 1.00,
  1.40, 0.30, 1.00,
